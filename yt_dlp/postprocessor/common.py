...
class PostProcessor(metaclass=PostProcessorMetaClass):
    ...
    def set_out_files(self):
        if not self._downloader:
            return None
        out_files = getattr(self._downloader, '_out_files', None)
        if out_files:
            return out_files
        ydl = getattr(self._downloader, 'ydl', None)
        return getattr(ydl, '_out_files', None) if ydl else None
    ...
    def _prepare_multiline_status(self, lines=1):
        if self._downloader:
            ...
            self._multiline.allow_colors = (
                self._multiline._HAVE_FULLCAP and not self._downloader.params.get('no_color'))
        else:
