...
def format_text(text, fmt):
    """
    @param text:    String to be formatted
    @param fmt:     String representation of formatting to apply in the form:
    ...
    if 'ON' in tokens:
        if tokens[-1] == 'ON':
            raise SyntaxError(f'Empty background format specified in {fmt!r}')
        ...
        if tokens[-1] == 'LIGHT':
            ...
            tokens.pop()
        if tokens[-1] != 'ON':
            raise SyntaxError(f'Invalid format {fmt.split(" ON ", 1)[1]!r} in {fmt!r}')
    ...
    if not tokens:
        ...
        if tokens:
            raise SyntaxError(f'Invalid format {" ".join(tokens)!r} in {fmt!r}')
