...
class RaiIE(RaiBaseIE):
    ...
    def _real_extract(self, url):
        content_info = self._extract_from_content_id(self._match_id(url), url)
        return content_info or self._legacy_real_extract(url)

    def _legacy_real_extract(self, url):
        # this code seems to be no longer useful. renamed to _legacy and created
        # a new _real_extract that gets info directly from the video_id
        # and uses _legacy only if it fails to retrieve infos
        self.to_screen('Using legacy extractor')
