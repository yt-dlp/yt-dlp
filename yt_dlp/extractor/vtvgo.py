...
class VTVgoVideoIE(InfoExtractor):
    _VALID_URL = [r'https://vtvgo\.vn/(kho-video|tin-tuc)/(?P<id>.*)\.html']
    _TESTS = [{
    ...
    def _real_extract(self, url):
        ...
        m3u8_url = self._search_regex(
            r'(https://vtvgo-vods\.vtvdigital\.vn/.*/index\.m3u8)',
            webpage, 'm3u8_url', fatal=False)
        if not m3u8_url:
        ...
        return {
            ...
            'description': self._og_search_description(webpage, default=None).strip(),
            'thumbnail': self._og_search_property('image', webpage, 'thumbnail', default=None),
            'formats': formats
        }
