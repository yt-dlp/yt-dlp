    ...
    def _real_extract(self, url):
        ...
        def process_language(container, base_url, lang_code, sub_name, client_name, query):
            ...
            for fmt in self._SUBTITLE_FORMATS:
                q = {**query, 'fmt': fmt, 'xosf': []}
                lang_subs.append({
                    'ext': fmt,
                    'url': urljoin('https://www.youtube.com', update_url_query(base_url, q)),
                    'name': sub_name,
                    'impersonate': True,
        ...
        # 1) read user-provided PO token:
        #    yt-dlp --extractor-args "youtube:po_token=web.subs+ACTUAL_TOKEN"
        user_po_token = None
        _ea = self._configuration_arg('po_token')  # this returns a list or None
        if _ea:
            raw = _ea[0]
            # allow "web.subs+TOKEN" or just "TOKEN"
            if '+' in raw:
                _, raw = raw.split('+', 1)
            user_po_token = raw.strip()
        ...
        for pr in prs:
            ...
            for caption_track in traverse_obj(pctr, ('captionTracks', lambda _, v: v['baseUrl'])):
                ...
                is_asr = caption_track.get('kind') == 'asr'
                requires_pot = (
                ...
                # 2) priority: user token
                if user_po_token:
                    pot_params.update({
                        'pot': user_po_token,
                        'potc': '1',
                        'c': innertube_client_name or 'WEB',
                    })
                ...
                # 3) still no token but track wants one → make a stub so --list-subs shows it
                if not pot_params and requires_pot:
                    ...
                    if is_asr:
                        code = remove_start(lang_code or orig_lang or 'und', 'a-') or 'und'
                        missing_po_caps.add(code)
                        automatic_captions.setdefault(code, []).append({
                            'ext': 'vtt',
                            'url': None,
                        ...
                        code = lang_code or orig_lang or 'und'
                        missing_po_subs.add(code)
                        subtitles.setdefault(code, []).append({
                            'ext': 'vtt',
                            'url': None,
                            'requires_po_token': True,
                            'name': lang_name,
                ...
                # 4) we have a token → build real URLs
                orig_lang = qs.get('lang', [None])[-1]
                lang_name = self._get_text(caption_track, 'name', max_runs=1)
                ...
                if not is_asr:
                    ...
                    # auto → put it in both automatic_captions AND subtitles
                    acode = remove_start(lang_code or orig_lang or 'und', 'a-') or 'und'
                    process_language(automatic_captions, base_url, acode, lang_name, client_name, pot_params)
                    process_language(subtitles, base_url, acode, lang_name, client_name, pot_params)
                ...
                for trans_code, trans_name in translation_languages.items():
                    ...
                    if not is_asr and trans_code != 'und':
        ...
        if (skipped_subs_clients and (need_subs_langs or need_caps_langs)) or missing_po_subs or missing_po_caps:
            self._report_pot_subtitles_skipped(
                video_id,
                True,
                msg=join_nonempty(
                    f'{video_id}: There are subtitles/captions that require a PO token.',
                    missing_po_subs and f'Subtitles for: {", ".join(sorted(missing_po_subs))}.',
                    missing_po_caps and f'Automatic captions for {len(missing_po_caps)} languages are missing.',
                    'Pass --extractor-args "youtube:po_token=web.subs+<TOKEN>" to download them.',
                    delim=' ',
                ),
            )
