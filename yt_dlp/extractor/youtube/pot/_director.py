    ...
    def get_provider_logger_and_settings(provider, logger_key):
        ...
        settings = {
            ...
            for k, v in traverse_obj(
                ie.get_param('extractor_args'),
                (f'{EXTRACTOR_ARG_PREFIX}-{provider.PROVIDER_KEY}', {dict.items}, ...),
                casesense=False)
        }
        return YoutubeIEContentProviderLogger(ie, logger_prefix, log_level=log_level), settings
