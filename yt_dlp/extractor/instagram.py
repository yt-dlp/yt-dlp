...
class InstagramBaseIE(InfoExtractor):
    ...
    def _login(self):
        ...
        login = self._download_json('https://www.instagram.com/accounts/login/ajax/', None, note='Logging in', headers=login_headers, data=urlencode_postdata({
        ...
        if login.get('two_factor_required'):
            tfa_info = login['two_factor_info']
            tfa_identifier, tfa_method, tfa_code = tfa_info['two_factor_identifier'], None, None
            if tfa_info.get('totp_two_factor_on'):
                tfa_method, tfa_code = 3, self._get_tfa_info('a 6-digit login code generated by the authentication app')
            elif tfa_info.get('sms_two_factor_on'):
                try:
                    sms_request = self._download_json('https://www.instagram.com/accounts/send_two_factor_login_sms/', None, note='Requesting SMS code', headers=login_headers, data=urlencode_postdata({
                        'username': username,
                        'identifier': tfa_identifier,
                    }))
                    if sms_request.get('status') != 'ok':
                        if sms_request.get('message'):
                            raise ExtractorError(f'Unable to request SMS code: {sms_request["message"]}')
                        raise ExtractorError('Unable to request SMS code')
                except ExtractorError as e:
                    if isinstance(e.cause, compat_HTTPError):
                        sms_request = self._parse_json(e.cause.read().decode(), None)
                        if sms_request.get('message'):
                            raise ExtractorError(f'Unable to request SMS code: {sms_request["message"]}')
                    raise e
            ...
            login = self._download_json('https://www.instagram.com/accounts/login/ajax/two_factor/', None, note='Sending TFA code', headers=login_headers, data=urlencode_postdata({
                ...
                'verification_method': tfa_method,
                'queryParams': '{}',
            }))
