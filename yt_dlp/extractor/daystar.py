...
class DaystarClipIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        sources = self._parse_json(self._search_regex(r'sources\:[^>](\[.*?\])', webpage_iframe, 'm3u8 source'),
                                   video_id=video_id, transform_source=js_to_json)
        ...
        for source in sources:
            ...
            if file and source.get('type') == 'm3u8':
                fmts, subs = self._extract_m3u8_formats_and_subtitles(urljoin('https://www.lightcast.com/embed/', file),
