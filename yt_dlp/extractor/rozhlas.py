...
class RozhlasVltavaIE(InfoExtractor):
    ...
    def _extract_video(self, entry):
        ...
        for audio in traverse_obj(entry, ('audioLinks', ..., {dict})):
            audio_url = audio.get('url')
            if not url_or_none(audio_url):
                continue
            ext = audio.get('variant')
            if ext == 'dash':
                formats.extend(self._extract_mpd_formats(
                    audio_url, audio_id, mpd_id=ext, fatal=False))
            elif ext == 'hls':
                formats.extend(self._extract_m3u8_formats(
                    audio_url, audio_id, 'm4a', m3u8_id=ext, fatal=False))
            else:
                formats.append({
                    'url': audio_url,
                    'ext': ext,
