...
class ManyVidsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_urls = self._download_json(f'https://www.manyvids.com/bff/store/video/{video_id}/private', video_id)[
            'data']
        ...
        return {
            ...
            'thumbnail': (
                url_or_none(traverse_obj(info, 'screenshot')) or url_or_none(traverse_obj(info, 'thumbnail'))),
            'view_count': (parse_count(traverse_obj(info, 'views'))),
            'like_count': (parse_count(traverse_obj(info, 'likes'))),
            'release_timestamp': (parse_iso8601(traverse_obj(info, 'launchDate'))),
            'duration': (parse_duration(traverse_obj(info, 'videoDuration'))),
            'tags': [t.get('label') for t in traverse_obj(info, 'tagList')],
        }
