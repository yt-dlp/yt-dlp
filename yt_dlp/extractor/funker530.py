...
def url_clean(video_id):
    ...
    try:
        ...
        link_match = re.search(r'"url":"(.*?)",', response.text)
...
class Funker530IE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        cleaned_link = url_clean(video_id)
        ...
        if rumble_url:
            info = {
                'url': rumble_url,
                'title': clean_html(get_element_by_class('main-video-title', webpage)),
                'info_dict': {
                    'ext': 'mp4',
                    'direct': True
                },
