...
from ..jsinterp import int_to_signed_32bit_int
from ..utils import (
    ExtractorError,
    clean_html,
    determine_ext,
...
class _ByteGenerator:
    def __init__(self, algo_id, seed):
        ...
        self._s = int_to_signed_32bit_int(seed)

    def _algo1(self, s):
        # LCG (a=1664525, c=1013904223, m=2^32)
        # Ref: https://en.wikipedia.org/wiki/Linear_congruential_generator
        s = self._s = int_to_signed_32bit_int(s * 1664525 + 1013904223)
        return s

    def _algo2(self, s):
        # xorshift32
        # Ref: https://en.wikipedia.org/wiki/Xorshift
        s = int_to_signed_32bit_int(s ^ (s << 13))
        s = int_to_signed_32bit_int(s ^ ((s & 0xFFFFFFFF) >> 17))
        s = self._s = int_to_signed_32bit_int(s ^ (s << 5))
        return s
    ...
    def _algo3(self, s):
        ...
        s = self._s = int_to_signed_32bit_int(s + 0x9e3779b9)
        s = int_to_signed_32bit_int(s ^ ((s & 0xFFFFFFFF) >> 16))
        s = int_to_signed_32bit_int(s * int_to_signed_32bit_int(0x85ebca77))
        s = int_to_signed_32bit_int(s ^ ((s & 0xFFFFFFFF) >> 13))
        s = int_to_signed_32bit_int(s * int_to_signed_32bit_int(0xc2b2ae3d))
        return int_to_signed_32bit_int(s ^ ((s & 0xFFFFFFFF) >> 16))

    def _algo4(self, s):
        # Custom scrambling function involving a left rotation (ROL)
        s = self._s = int_to_signed_32bit_int(s + 0x6d2b79f5)
        s = int_to_signed_32bit_int((s << 7) | ((s & 0xFFFFFFFF) >> 25))  # ROL 7
        s = int_to_signed_32bit_int(s + 0x9e3779b9)
        s = int_to_signed_32bit_int(s ^ ((s & 0xFFFFFFFF) >> 11))
        return int_to_signed_32bit_int(s * 0x27d4eb2d)

    def _algo5(self, s):
        # xorshift variant with a final addition
        s = int_to_signed_32bit_int(s ^ (s << 7))
        s = int_to_signed_32bit_int(s ^ ((s & 0xFFFFFFFF) >> 9))
        s = int_to_signed_32bit_int(s ^ (s << 8))
        s = self._s = int_to_signed_32bit_int(s + 0xa5a5a5a5)
        return s

    def _algo6(self, s):
        # LCG (a=0x2c9277b5, c=0xac564b05) with a variable right shift scrambler
        s = self._s = int_to_signed_32bit_int(s * int_to_signed_32bit_int(0x2c9277b5) + int_to_signed_32bit_int(0xac564b05))
        s2 = int_to_signed_32bit_int(s ^ ((s & 0xFFFFFFFF) >> 18))
        shift = (s & 0xFFFFFFFF) >> 27 & 31
        return int_to_signed_32bit_int((s2 & 0xFFFFFFFF) >> shift)

    def _algo7(self, s):
        # Weyl Sequence (k=0x9e3779b9) + custom multiply-xor-shift mixing function
        s = self._s = int_to_signed_32bit_int(s + int_to_signed_32bit_int(0x9e3779b9))
        e = int_to_signed_32bit_int(s ^ (s << 5))
        e = int_to_signed_32bit_int(e * int_to_signed_32bit_int(0x7feb352d))
        e = int_to_signed_32bit_int(e ^ ((e & 0xFFFFFFFF) >> 15))
        return int_to_signed_32bit_int(e * int_to_signed_32bit_int(0x846ca68b))
