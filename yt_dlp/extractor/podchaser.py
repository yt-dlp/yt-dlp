...
class PodchaserIE(InfoExtractor):
    ...
    _VALID_URL = _VALID_URL_BASE + r'(?P<podcast_id>\d+)/episodes/[\w-]+-(?P<id>\d+)'
    _TESTS = [{
    ...
    def _extract_episode(self, podcast, episode):
        return {
    ...
    def _real_extract(self, url):
        ...
        podcast = self._download_json(self._API_BASE + 'podcasts/%s' % podcast_id, podcast_id)
        episode = self._download_json(self._API_BASE + 'episodes/%s' % episode_id, episode_id)
        return self._extract_episode(podcast, episode)


class PodchaserFeedIE(PodchaserIE):
    IE_NAME = 'Podchaser:feed'
    _VALID_URL = PodchaserIE._VALID_URL_BASE + r'(?P<id>\d+)'
    _TESTS = [{
    ...
    def _fetch_page(self, podcast_id, podcast, page):
        ...
        json_response = self._download_json(
            self._API_BASE + 'list/episode', podcast_id,
            headers={'Content-Type': 'application/json;charset=utf-8'},
            data=json.dumps(params).encode())
        episodes = json_response.get('entities')
        for episode in episodes:
            yield self._extract_episode(podcast, episode)

    def _real_extract(self, url):
        podcast_id = self._match_valid_url(url).group('id')
        podcast = self._download_json('https://api.podchaser.com/podcasts/%s' % podcast_id, podcast_id)

        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, podcast_id, podcast), self._PAGE_SIZE)

        return self.playlist_result(
            entries,
            playlist_id=str(podcast.get('id')),
            playlist_title=podcast.get('title'),
            playlist_description=podcast.get('description'))
