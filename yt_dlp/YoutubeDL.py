...
class YoutubeDL:
    ...
    def __init__(self, params=None, auto_init=True):
        ...
        if (sys.platform != 'win32'
            ...
            self.report_warning(
                f'Assuming {self._format_err("--restrict-filenames", self.Styles.OPTION)} since file system encoding '
                'cannot encode all characters. '
                f'Set the {self._format_err("LC_ALL", self.Styles.EMPHASIS)} environment variable to fix this.')
    ...
    def warn_if_short_id(self, argv):
        ...
        if idxs:
            ...
            self.report_warning(
                ...
                f'Use {self._format_err("--", self.Styles.DEMAND)} to separate parameters and URLs, like this:\n'
    ...
    def to_stdout(self, message, skip_eol=False, quiet=None):
        ...
        if quiet is not None:
            self.deprecation_warning(
                '"YoutubeDL.%(to_stdout)s" no longer accepts the argument quiet. Use "YoutubeDL.'
                '%(to_screen)s" instead' % {
                    'to_stdout': self._format_err("to_stdout", self.Styles.OPTION),
                    'to_screen': self._format_err("to_screen", self.Styles.DEMAND)
                })
        self._write_string(
    ...
    def raise_no_formats(self, info, forced=False, *, msg=None):
        ...
        err_msg = msg if has_msg or not has_drm else 'This video is DRM protected'
        if forced or not ignored:
            raise ExtractorError(err_msg, video_id=info['id'], ie=info['extractor'],
        ...
        if warn:
            if not self.params.get('paths'):
                ...
                self.report_warning(f'{self._format_err("--paths", self.Styles.OPTION)} is ignored since '
                                    'an absolute path is given in output template',
                                    only_once=True)
    ...
    def __process_playlist(self, ie_result, download):
        ...
        for i, entry_tuple in enumerate(entries, 1):
            ...
            if failures >= max_failures:
                self.report_error('Skipping the remaining entries in playlist "%(playlist)s" since '
                                  '%(failures)s items failed extraction' % {
                                      'playlist': self._format_err(playlist, self.Styles.ID),
                                      'failures': self._format_err(failures, self.Styles.EMPHASIS)
                                  })
                break
        ...
        if is_video:
            ...
            if not title:
                if title == '':
                    ...
                    self.report_warning('Extractor failed to obtain "%(title)s". '
                                        'Creating a generic title instead' % {
                                            'title': self._format_err("title", self.Styles.KEY)})
    ...
    def process_video_result(self, info_dict, download=True):
        ...
        while True:
            if interactive_format_selection:
                req_format = input(
                    self._format_screen('\nEnter format selector: ', self.Styles.DEMAND))
                try:
                    format_selector = self.build_format_selector(req_format)
    ...
    def process_info(self, info_dict):
        ...
        def _write_link_file(link_type):
            ...
            try:
                ...
                self.report_error('Cannot write internet shortcut %s' %
        ...
        if self.params.get('skip_download'):
            ...
            try:
                    ...
                    if dl_filename is not None:
                        ...
                        if temp_filename == '-':
                            reason = ('using a downloader other than %s' %
                                      ...
                                      f'but {self._format_err("ffmpeg", self.Styles.REQUIREMENT)} is not installed')
                            self.report_warning(
                ...
                self.report_error('unexpected error happened: '
    ...
    def print_debug_header(self):
        ...
        if logger:
            ...
            write_string('[%(debug)s] %(encodings)s\n' % {
                'debug': self._format_err(*self.Channels.DEBUG),
                'encodings': encoding_str}, out=self._out_files['error'], encoding=None)
            write_debug = lambda msg: self._write_string('[%(debug)s] %(msg)s\n' % {
                'debug': self._format_err(*self.Channels.DEBUG),
                'msg': msg}, out=self._out_files['error'])
    ...
    def _write_description(self, label, ie_result, descfn):
        ...
        if not self.params.get('writedescription'):
            ...
            self.to_screen('[%(info)s] %(title)s description is already present' % {
                'info': self._format_screen(*self.Channels.INFO),
                'title': label.title()})
    ...
    def _write_subtitles(self, info_dict, filename):
        ...
        for sub_lang, sub_info in subtitles.items():
            ...
            try:
                ...
                msg = 'Unable to download video subtitles for %(lang)s: %(err)s' % {
                    'lang': self._format_err(repr(sub_lang), self.Styles.KEY),
                    'err': self._format_err(err, self.Styles.ERROR)
                }
