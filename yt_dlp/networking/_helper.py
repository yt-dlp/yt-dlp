...
def make_ssl_context(
    ...
    if legacy_support:
        ...
        # Keep TLS 1.2+ across runtimes while avoiding the broader DEFAULT policy for non-legacy
        # requests. On OpenSSL 3.x, the older Python 3.10-style backport can be blocked by some
        # sites, but switching fully to DEFAULT re-enables older SHA1-based suites that legacy_ssl
        # is supposed to cover. Narrowly add the OpenSSL 3 DHE ChaCha20 suite instead.
        if ssl.OPENSSL_VERSION_INFO >= (3, 0, 0):
            ...
            # Backport the Python 3.10 cipher policy for older OpenSSL builds.
            # See: https://github.com/yt-dlp/yt-dlp/issues/4627
            # See: https://github.com/yt-dlp/yt-dlp/pull/5294
            context.set_ciphers(
