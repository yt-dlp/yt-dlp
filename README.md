...
* `player_skip`: Skip some network requests that are generally needed for robust extraction. One or more of `configs` (skip client configs), `webpage` (skip initial webpage), `js` (skip js player), `initial_data` (skip initial data. When skipping webpage, configs, and initial_data at the same time, the result JSON data will lose metadata such as uploader, description, view_count, comment_count, etc. This option is only for those who just want to get downloadable formats.). While these options can help reduce the number of requests needed or avoid some rate-limiting, they could cause some issues. See [#860](https://github.com/yt-dlp/yt-dlp/pull/860) for more details
