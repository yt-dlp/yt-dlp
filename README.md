...
#### vimeo
* `client`: Client to extract video data from. The currently available clients are `android`, `ios`, `macos` and `web`. Only one client can be used. The `web` client is used by default, and only works with account cookies or login credentials. The `android`, `ios`, and `macos` clients only work with previously cached OAuth tokens
