...
This release channel can be changed by using the `--update-to` option. `--update-to` can also be used to upgrade or downgrade to specific tags from a channel. When using `--update-to`, it will only update if the new version is higher than the current version. This will be the case even when changing channels.
