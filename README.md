...
* `use_ad_playback_context`: Skip preroll ads to eliminate the mandatory wait period before download. Results in premium formats not being served. On clients other than `web`, `web_safari`, and `mweb` can result in 403 errors on some videos. Either `true` or `false` (default)
