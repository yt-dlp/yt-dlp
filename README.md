...
* `force_js_variant`: The player javascript variant to use for signature and nsig deciphering. The available clients variants are `base`, `tce`, `tv`, `es6`, `phone`, `tablet`. Only `base` is recommended as a possible workaround; the others are mainly for debugging purposes. The default is to use what is prescribed by the site, and can be selected with `false`
