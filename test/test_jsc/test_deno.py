...
@pytest.mark.skipif(not yt_dlp_jsc, reason='yt-dlp-jsc not available')
class TestDenoJCP:
    TESTS = [
    ...
    def jcp(self, ie, logger) -> DenoJCP:
        return DenoJCP(ie, logger, settings={'debug': ['true']})

    def test_bulk_requests(self, jcp: DenoJCP):
        assert list(jcp.bulk_solve(self.TESTS)) == self.RESPONSES
